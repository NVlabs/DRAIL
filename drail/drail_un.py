...
class Discriminator(nn.Module):
    ...
    def diffusion_loss(self, sa_pair, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, n_steps):
        ...
        output = self.model(x, t.squeeze(-1))
        
        return (e - output).square().mean(dim=1, keepdim=True)
        #return torch.unsqueeze(torch.mean(e - output, dim=1), 1)

    def diffusion_loss_fn(self, sa_pair):
        diff_loss = self.diffusion_loss(sa_pair, self.alphas_bar_sqrt, self.one_minus_alphas_bar_sqrt, self.n_steps)
    ...
    def forward(self, state, action):
        ...
        loss = self.diffusion_loss_fn(state_action)
...
class DRAIL_UN(NestedAlgo):
    def __init__(self, agent_updater=PPO(), get_discrim=None):
        super().__init__([DRAIL_UNDiscrim(get_discrim, policy=agent_updater), agent_updater], 1)

class DRAIL_UNDiscrim(BaseIRLAlgo):
    ...
    def _compute_disc_val(self, state, action):
        disc_output = self.discrim_net(state, action)
