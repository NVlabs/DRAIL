...
class GAIL_BC(NestedAlgo):
    def __init__(self, agent_updater=PPO_BC(), get_discrim=None):
...
class GailDiscrim(BaseIRLAlgo):
    ...
    def _compute_expert_loss_revise(self, expert_d, expert_batch):
        return - expert_d.mean()

    def _compute_agent_loss_revise(self, agent_d, agent_batch):
        self.agent_loss = -1 - torch.log(-agent_d.mean())
        return -1 - torch.log(-agent_d.mean())
    ...
    def _update_reward_func(self, storage, gradient_clip=False, t=1):
        ...
        for epoch_i in range(self.args.n_gail_epochs):
            for expert_batch, agent_batch in zip(expert_sampler, agent_sampler):
                ...
                if self.args.revise:
                    expert_loss = self._compute_expert_loss_revise(expert_d, expert_batch)
                    agent_loss = self._compute_agent_loss_revise(agent_d, agent_batch)                
                else:
    ...
    def get_add_args(self, parser):
        ...
        parser.add_argument('--reward-type', type=str, default='airl', help="""
                ...
                not change training.
                """)
        parser.add_argument('--revise', type=str2bool, default=False)
        parser.add_argument('--discrim-depth', type=int, default=1)
